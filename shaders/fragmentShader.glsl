...
void main() {
	...
	if (showBlack) {
		...
		gl_FragColor = vec4(ambientColor + lambertian * color + specular * specColor, 1.0);
	} else {
		gl_FragColor = vec4(ambientColor + lambertian * color + specular * specColor, alpha);
	}
}